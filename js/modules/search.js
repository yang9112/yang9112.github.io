...
(function() {
    ...
    postElements.forEach(function(post) {
      ...
      if (titleElement && linkElement) {
        const postData = {
          ...
          excerpt: getExcerpt(contentElement),
  ...
  function getExcerpt(contentElement, maxLength = 150) {
    ...
    searchIndex.forEach(function(post) {
      ...
      if (score > 0) {
        results.push({
          ...post,
          score: score,
          highlightedTitle: highlightTerms(post.title, terms),
          highlightedExcerpt: highlightTerms(post.excerpt, terms)
        });
      }
    });
  ...
  function initSearch() {
    ...
    if (existingSearchForm) {
      existingSearchForm.addEventListener('submit', function(e) {
        e.preventDefault();
        const modal = showSearchModal();
        setupSearchHandlers(modal);
      });
  ...
  function setupSearchHandlers(modal) {
    const input = modal.querySelector('.search-input');
  ...
  window.Search = {
    ...
    isReady: function() { return isSearchReady; }
  };
